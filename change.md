...
2026/01/02 ziyouren55
引入Redis相关，并且将redis和mySQL数据库都打包进了docker，现在能通过compose一键拉起了
利用Redis实现了单点登录，Redis会存储当前登录的用户信息，并且系统会管理token的消亡和创建 
添加了API统计 ，对于面向切面编程的内容可以再往下探索一点
由于重建了数据库，所以在测试订单流程的时候订单号会和之前数据库下的订单号相同，原先使用订单号作为支付订单的一个重要标识，导致在支付的时候支付宝那边有重复数据，现已替换为更加严密随机的逻辑
修复了游客访问论坛主页面时的后端抛出错误的问题，现在能够正常访问了，访问到详情页的时候会导向登录 

2026/01/03 ziyouren55
利用Redis里的hash和zset数据结构完成了热门商品推荐相关逻辑 
修复了关于单点登录的一个后端的处理问题，原先是广播现在是定向发送了，不会产生不同账号之间的干扰 
为附近推荐在游客状态下的访问添加了登陆的路由导航
