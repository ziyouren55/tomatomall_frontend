...
import { memberService } from '@/api/services'  // 导入memberService
...
const fetchMemberLevels = async () => {
  loading.value = true
  try {
    const response = await memberService.getAllLevels()
    memberLevels.value = response.data || []
  } catch (error) {
    console.error('获取会员等级失败:', error)
    alert('获取会员等级失败: ' + error.message)
    memberLevels.value = []
  } finally {
    loading.value = false
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await memberService.updateLevel(editingLevel.value.id, payload)
      alert('更新成功！')
    } else {
      await memberService.createLevel(payload)
      alert('创建成功！')
    }
...
const deleteSelected = async () => {
  ...
  try {
    for (const levelId of selectedLevels.value) {
      await memberService.deleteLevel(levelId)
    }
