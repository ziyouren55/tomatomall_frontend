...
// 跳转到商品详情页
const goToProduct = (productId: number | string | undefined) => {
  if (productId) {
...
// 填充表单 - 兼容不同的字段名
const fillForm = (ad: Advertisement) => {
  form.title = ad.title || ''
  form.content = ad.content || ''
  form.imgUrl = ad.image_url || ad.imageUrl || ''
  form.productId = String(ad.product_id || ad.productId || '')
}
...
const saveAdvertisement = async () => {
  ...
  try {
    ...
    const errorMsg = errorData?.message || '保存广告失败'
...
const batchDelete = async () => {
  ...
  try {
    ...
    const axiosError = error as AxiosError
    const errorData = axiosError.response?.data as any
    const errorMsg = errorData?.message || '删除广告失败'
    showMessage(errorMsg, 'error')
  } finally {
    loading.value = false
  }
...
const handleImageError = (event: Event) => {
  ...
  target.src = 'https://via.placeholder.com/400x300?text=图片加载失败'
}
