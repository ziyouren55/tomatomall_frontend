  ...
  export default {
    ...
    setup() {
      ...
      onMounted(async () => {
        ...
        advertisementModal = new bootstrap.Modal(document.getElementById('advertisementModal'));
      ...
      const fetchAdvertisements = async () => {
        ...
        try {
          ...
          if (response.data && response.data.code === 200) {
            ...
            console.error('Failed to fetch advertisements:', response.data?.msg);
          }
        } catch (error) {
      ...
      const deleteAdvertisement = async () => {
        try {
          const response = await advertisementService.deleteAdvertisement(adToDelete.value.id);
          if (response.data && response.data.code === 200) {
            fetchAdvertisements();
            confirmDeleteModal.hide();
          } else {
            console.error('Failed to delete advertisement:', response.data?.msg);
          }
        } catch (error) {
      ...
      const saveAdvertisement = async (advertisementData) => {
        try {
          let response;
          if (editMode.value) {
            response = await advertisementService.updateAdvertisement(advertisementData);
          } else {
            response = await advertisementService.createAdvertisement(advertisementData);
          }
          ...
          if (response.data && response.data.code === 200) {
            ...
            console.error('Failed to save advertisement:', response.data?.msg);
            alert(`操作失败: ${response.data?.msg || '未知错误'}`);
          }
        } catch (error) {
